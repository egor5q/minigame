...
pokemons=['Подрочу','Дрочемыш','Хуебес','Писькозавр','Кончефлыга','Блядун','Хуйдрочу','Подрочилла','Дрочь','Залупяндрий','Еблодок',
         ...
         'Спермокрыс','Крысоёб','Спермоед','Жопныйглист','Пиздомон','Великая Дрочила','Ебальничек','Вонючий хуй','Глубокая щель',
         'Ебаный насос','Покетрах','Трахочу','Пенис','Залупер','Хуймондок','Большой ебун','Глиномес','Рагнарос','Нахуймон',
         'Спермокач','Наспермонд','Член лося','Член коня','Дрочиловка','Недодрочер','Безхуёныч','Долбоящер','Блядохуй',
         'АнальныйЩит','Пенис детров','Братишка','Анальный брат','Охуяндий','Хуила','Выблядок','Подспермон','Переспермыч',
         'Дохуяшка','Инвалид','Бля фантазии не хватает','12 залуп оушена','Кросстрахчу','Пиздаболер','Перепиздун','Хуймок',
 ...
 if x==0:
  if m.from_user.id not in pokeban:
   ...
   if x!=None:
    ...
    t=threading.Timer(1, unpoke, args=[m.from_user.id])
    t.start()
   else:
